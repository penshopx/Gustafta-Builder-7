...
- **SKK ABD v1.1 Upgrade (53 agen — COMPLETE)**: Semua agen SKK murni diupgrade dengan blok ABD v1.1 via `scripts/upgrade-skk-abd.ts`. Marker: `SKK_ABD_v1.1_UPGRADED`. Blok SKK-spesifik: CATATAN REGULASI WAJIB (Permen PUPR 9/2023 + SKKNI acuan utama, KKNI L1-9, LSP/BNSP/LPJK), POLA KERJA v2.0, STATE MACHINE (INIT→ELICIT→PLAN→DELIVER), ANTI-BLOCK DOCTRINE + HEURISTIK DEFAULT SKK (jenjang default KKNI L6 jika tidak ada info, LSP relevan per bidang), GUARDRAILS SKK (DILARANG janjikan SKK terbit/lulus uji, DILARANG pinjam nama tenaga ahli). Cakupan: SKK Coach 13 HUB (438–543), SKK AJJ (177–205), SKKNI EDU/QUIZ/PORTO/REG (649–672), Proses Sertifikasi SKK, Siap Uji, AJJ Nirkertas, AGENT-SKKMATCH.
