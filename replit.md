...
The platform employs a 5-level modular hierarchical structure (Series → Core → Big Idea/Orkestrator → Toolbox → Agent) for Jasa Konstruksi compliance across 27+ series, totaling approximately 312 chatbot agents. The newest series (sortOrder 11) is **"Odoo ERP BUJK — Implementasi & Operasional Konstruksi Indonesia"** (slug: `odoo-erp-bujk-implementasi`), built on the implementation blueprint of Wuryanto Kusdjali (SB NET's Space). It contains 6 chatbot agents in a Hub-and-Spoke architecture: 1 Orchestrator (ODOO-BUJK-ORCHESTRATOR) + 5 specialists — AGENT-ODOO-PROJECT-BOQ (WBS/bujk_boq AHSP/bujk_mc/Kurva-S/MRP/Quality/BIM IFC), AGENT-ODOO-FINANCE (l10n_id/e-Faktur Coretax/PPh Final PP 9/2022/PSAK 71-72-73/Retensi/Cashflow), AGENT-ODOO-HR-SMKK (bujk_skk KKNI 1-9/PJ-BUJK Matrix/BPJS UU 6/2023/bujk_smkk Permen PUPR 10/2021), AGENT-ODOO-SCM-INTEGRASI (Purchase/bujk_subcon back-to-back/Inventory per proyek/bujk_equipment_rental/connector OSS-RBA/SIKI/SIJK), and AGENT-ODOO-PMO-RACI (Roadmap 12 bulan/RACI 7-stakeholder/Anggaran K-M-B/ADKAR/Vendor compare). It is operationally complementary to the older "Odoo untuk Jasa Konstruksi" series (sortOrder 4) which focuses on AI-assisted Readiness/Blueprint/Governance. A key feature is "Project Brain," providing contextual data for chatbots, enabling specialized "Mini Apps" for tasks like project snapshots, decision summaries, and risk assessments. A "Deliverables" panel allows builders to define output types for each agent, with various pre-built bundles and quick-action chat buttons.
