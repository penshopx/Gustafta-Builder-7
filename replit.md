...
- **Product Landing Pages**: Per-chatbot marketing/sales landing pages at `/product/:agentId`. Configurable via "Landing Page" panel in dashboard. Sections: Hero (headline + CTA), Problem Agitation (pain points), Solution Framing, Demo/Preview, Outcome Benefits, Testimonials, Pricing (from conversionOffers), Risk Reversal/Guarantees, Authority/Credentials, FAQ, and multiple CTAs linking to chat. Agent fields: landingPageEnabled, landingHeroHeadline, landingHeroSubheadline, landingHeroCtaText, landingPainPoints, landingSolutionText, landingBenefits, landingDemoItems, landingTestimonials, landingFaq, landingAuthority, landingGuarantees.
- **Marketing Kit**: Comprehensive ad copywriting and creative prompt management via "Marketing Kit" panel in dashboard. Features: (1) AI-generated ad copy for Meta Ads, Instagram, Google Ads, TikTok, LinkedIn with platform-specific specs; (2) Image hook prompts for AI image generators (Midjourney, DALL-E) with style/platform targeting; (3) Video reel prompts with duration/platform options; (4) Meta Ads Pixel integration with auto-injection on public chat and landing pages, tracking PageView, ViewContent, Lead, and InitiateCheckout events. Agent fields: adCopies (jsonb record by platform), imageHookPrompts (jsonb array), videoReelPrompts (jsonb array), metaPixelId (text). API endpoints: POST `/api/agents/:id/generate-ad-copy`, POST `/api/agents/:id/generate-creative-prompts`.
