...
- **Chat UI Markdown Renderer (Apr 2026)**: Shared `MessageContent` component at `client/src/lib/format-message.tsx` renders chat assistant messages with light markdown — `**bold**`, `*italic*`, `` `code` ``, `~~strike~~`, `[link](url)` (with protocol allowlist: http/https/mailto/tel + relative paths), `# ## ###` headings, `- * +` bullet lists, `1.` ordered lists, `>` blockquotes, `---` separators, ` ``` ` fenced code blocks, and **pipe tables** (`| col | col |\n|---|---|\n| cell | cell |`) with column alignment (`:--`, `--:`, `:--:`) rendered as proper `<table>` with bordered cells, zebra rows, and horizontal scroll. Used in: `modul-chat.tsx`, `chat-popup.tsx` (widget), `embed-chat.tsx` (iframe embed), `chat-console-panel.tsx` (studio test), `trial-chat.tsx`, and `agent-chat.tsx` (via `formatMessageContent` wrapper). Replaces previous `cleanMarkdown` strippers that produced flat text without emphasis. Uses safe React JSX (no `dangerouslySetInnerHTML`).
