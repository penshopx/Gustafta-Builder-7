...
- **Chaesa AI Studio Bridge**: Adapter that maps Gustafta chatbot configuration into the schema used by Chaesa AI Studio (https://smart-ebook-builder-7-1.replit.app/) — an external "AI Prompt Generator" for ebook ecosystems. Output bundle contains three sections: `projectData` (industry, topik, judul, target, level, tujuan, painPoint, bigIdea, hasilRiset, produk, language, outputFormat, tone, writingStyle, aiCharacter), `botBuilder` GPT_BUILDER (botName, botRole, botPersonality, botPersonaDetail, botLanguage, botAudience, botAvoidTopics, botSystemPrompt), and `packConfig` PROMPT_PACK defaults. Heuristic Indonesian-language industry classifier auto-picks the closest of 24 Chaesa industries from agent category/toolbox text; KB content is summarised into the `hasilRiset` field. Endpoint: `GET /api/agents/:id/export/chaesa[?download=1]` (auth-gated via `assertCanPreviewAgentPrompt`). UI: violet-accent card in Studio → Export tab — "Preview Field" opens a per-field copy-to-clipboard dialog grouped by section, plus one-click JSON download and external launcher to Chaesa. Source files: `server/lib/chaesa-exporter.ts`.
