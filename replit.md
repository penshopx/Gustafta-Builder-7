...
-   **Idempotent Seeder**: `server/seed-skk-sipil-wave2.ts` pola sama dengan Wave 1 (lookup series by name/slug, getBigIdeas dengan series filter, upsert by toolbox/agent name). Terdaftar di `server/index.ts` registry (`M_skkSipilWave2`) dan `seedTasks` setelah `SKK Sipil Wave1`. Series 109 sekarang punya 10 modul total (5 Wave 1 Bangunan Gedung + 5 Wave 2 Manajemen).
