...
async function callGemini(agent: any, attempt: number): Promise<any> {
...
async function fillOne(client: any, agent: any) {
  console.log(`\n→ ${agent.series} | ${agent.name}`);
  const cat = SERIES_CATEGORY[agent.series] || "regulasi";

  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      const { text, parsed: rawParsed } = await callGemini(agent, attempt);
      ...
      const finalOutcome = validOutcomes.includes(outcome) ? outcome : "user_education";
      ...
      console.log(`  attempt ${attempt} error: ${e.message.substring(0, 200)}`);
...
async function run() {
  ...
  try {
    ...
    `);
    console.log(`Found ${agents.length} stuck agents`);
    let ok = 0;
