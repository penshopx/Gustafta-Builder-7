...
const BIGIDEAS = [
  {
    ...
    type: "problem",
    description: "Tiga domain hukum fundamental: analisis tindak pidana (KUHP 2023), sengketa perdata & wanprestasi (KUHPerdata), dan strategi beracara di pengadilan.",
    agentIds: ["pidana", "perdata", "litigasi"],
    sortOrder: 1,
  },
  {
    ...
    type: "solution",
    description: "Empat domain hukum bisnis: korporasi & pasar modal (UUPT/OJK), ketenagakerjaan (UU Cipta Kerja), pertanahan (BPN/UUPA), perpajakan (DJP/UU HPP), dan kepailitan/PKPU.",
    agentIds: ["korporasi", "ketenagakerjaan", "pertanahan", "pajak", "kepailitan"],
    sortOrder: 2,
  },
  {
    ...
    type: "process",
    description: "Tiga domain lanjutan: riset yurisprudensi MA/MK, legal drafting kontrak & dokumen hukum, serta hukum digital (UU PDP, fintech, IP, e-commerce).",
...
export async function seedLexCom(userId: string): Promise<{ created: number; skipped: boolean }> {
  ...
  if (existing) {
    log("[Seed LexCom] Series sudah ada, skip.");
    return { created: 0, skipped: true };
  }
  ...
  const series = await storage.createSeries(
    {
      ...
      slug: SERIES_SLUG,
      description:
        "Ekosistem AI Hukum Indonesia — **1 Orchestrator (Lex) + 12 Agen Spesialis** yang mencakup seluruh cabang hukum: Pidana (KUHP 2023), Perdata, Korporasi, Ketenagakerjaan (UU Cipta Kerja), Pertanahan (BPN), Pajak (DJP/UU HPP), Yurisprudensi MA/MK, Legal Drafting, Litigasi & Arbitrase, Kepailitan/PKPU, MultiClaw, dan OpenClaw (hukum digital). Setiap respons menggunakan format IRAC+ dengan sitasi pasal eksplisit dan disclaimer otomatis.",
      tagline: "12 Agen Spesialis Hukum Indonesia — dari Pidana hingga Hukum Digital",
      coverImage: "",
      color: "#7c3aed",
      category: "legal",
      tags: [
        "hukum", "legal", "lexcom", "ai hukum", "kuhp 2023", "perdata", "pidana",
        "korporasi", "pajak", "ketenagakerjaan", "pertanahan", "yurisprudensi",
        "litigasi", "kepailitan", "hukum digital", "multi-agent", "irac",
      ],
      language: "id",
      isPublic: true,
      isFeatured: true,
